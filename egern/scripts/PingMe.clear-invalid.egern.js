...
function normalizeHeaderNameMap(headers) {
  ...
  if (headers && typeof headers.get === 'function') {
    ['User-Agent', 'Accept', 'Accept-Language', 'Content-Type'].forEach(k => {
...
async function loadStore(ctx) {
  if (ctx.storage && typeof ctx.storage.getJSON === 'function') {
    const saved = await ctx.storage.getJSON(storeKey);
    if (saved) {
      if (!saved.accounts) saved.accounts = {};
      if (!Array.isArray(saved.order)) saved.order = Object.keys(saved.accounts);
      return saved;
    }
  }
  const raw = await ctx.storage.get(storeKey);
...
async function saveStore(ctx, store) {
  if (ctx.storage && typeof ctx.storage.setJSON === 'function') {
    await ctx.storage.setJSON(storeKey, store);
    return;
  }
  await ctx.storage.set(storeKey, JSON.stringify(store));
}
...
async function httpGet(ctx, url, headers) {
  try {
    ...
    throw { error: err && (err.message || String(err)) };
  }
}

function isInvalidAccountMessage(message) {
  return /register|login|unauthorized|not\s*authorized|not\s*login|token|auth|登录|注册|未登录|请登录/i.test(message || '');
}

async function checkAccount(ctx, acc, index, total) {
  const tag = `[账号${index + 1}/${total} ${acc.alias || acc.id}]`;
  const ua = buildUA(acc.baseUA, acc.uaSeed);
  const headers = buildHeaders(acc.capture, ua);

  try {
    const res = await httpGet(ctx, buildUrl('queryBalanceAndBonus', acc.capture), headers);
    const data = JSON.parse(res.body || '{}');
    if (data.retcode === 0) {
      const balance = data.result && data.result.balance !== undefined ? data.result.balance : '?';
      return { invalid: false, text: `${tag} 保留：余额 ${balance} Coins` };
    }

    const message = data.retmsg || '未知错误';
    if (isInvalidAccountMessage(message)) {
      return { invalid: true, text: `${tag} 删除：${message}` };
    }
    return { invalid: false, text: `${tag} 保留：${message}` };
  } catch (err) {
    return { invalid: false, text: `${tag} 保留：检查失败（${err.error || String(err)}）` };
  }
}
...
export default async function(ctx) {
  ...
  if (!ids.length) {
    const message = '当前没有已保存账号';
    await notify(ctx, '⚠️ 无账号可清理', message);
    return buildResultWidget('PingMe：无账号可清理', message);
  }
  ...
  for (let idx = 0; idx < ids.length; idx++) {
    const id = ids[idx];
    const result = await checkAccount(ctx, store.accounts[id], idx, ids.length);
    lines.push(result.text);
    if (result.invalid) removed.push(id);
    else kept.push(id);
  }

  removed.forEach(id => delete store.accounts[id]);
  store.order = kept;
  await saveStore(ctx, store);

  const summary = `已删除 ${removed.length} 个失效账号，保留 ${kept.length} 个账号。`;
  const body = [summary, ...lines].join('\n');
  await notify(ctx, '🧹 清理完成', body);
  return buildResultWidget('PingMe：清理完成', body);
}
