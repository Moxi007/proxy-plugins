...
function runAccount(ctx, acc, index, total) {
  ...
  function fetchApi(path, useFakeId) {
    ...
    const attempt = (n) => httpGet(ctx, buildUrl(path, acc.capture, overrideId), headers)
      .catch(err => {
...
export default async function(ctx) {
  ...
  await notify(ctx, existed ? '🔄 账号参数已更新' : '✅ 新账号已入库', `${alias}（id:${fp}）\n当前账号总数：${total}`);
